...
sealed class VoiceIntent(val type: String, val rawText: String) {
    ...
    data class SetReminder(val time: String, val content: String, val text: String) : VoiceIntent("set_reminder", text)
...
class IntentRegistry(private val context: Context, private val voiceEngine: VoiceEngine) {
    ...
    private val patterns = listOf(
        Regex("""(?:播放|听|放)(?:一)?[下]??.*?(?:歌|音乐|歌曲|曲)") to { text ->
            VoiceIntent.PlayMusic(text.trim(), text)
        },
        Regex("""(?:停|停止|暂停|别播)(?:播放|音乐|歌)") to { text ->
            VoiceIntent.StopMusic(text)
        },
        Regex("""(?:下一|切到下一|换)(?:首|首歌|音乐)") to { text ->
            VoiceIntent.NextSong(text)
        },
        Regex("""(?:上一|切到上一|换上一)(?:首|首歌|音乐)") to { text ->
            VoiceIntent.PrevSong(text)
        },
        Regex("""音量(?:调到|设为|调至)?(\d+)[percent]?%?)") to { text, match ->
            val level = match?.groups?.get(1)?.value?.toIntOrNull() ?: 50
            VoiceIntent.SetVolume(level.coerceIn(0, 100), text)
        },
        Regex("""(?:移动|转移|搬到|移到)(?:从)?(.+?)(?:到|至)(.+?)") to { text, match ->
            val from = match?.groups?.get(1)?.value?.trim() ?: ""
            val to = match?.groups?.get(2)?.value?.trim() ?: ""
            VoiceIntent.MoveFile(from, to, text)
        },
        Regex("""(?:复制|拷贝)(?:从)?(.+?)(?:到|至)(.+?)") to { text, match ->
            val from = match?.groups?.get(1)?.value?.trim() ?: ""
            val to = match?.groups?.get(2)?.value?.trim() ?: ""
            VoiceIntent.CopyFile(from, to, text)
        },
        Regex("""(?:删除|删掉|移除)(.+?)") to { text, match ->
            val path = match?.groups?.get(1)?.value?.trim() ?: ""
            VoiceIntent.DeleteFile(path, text)
        },
        Regex("""(?:打开|启动|运行)(.+?)(?:应用|APP|app)?") to { text, match ->
            val appName = match?.groups?.get(1)?.value?.trim() ?: ""
            VoiceIntent.OpenApp(appName, text)
        },
        Regex("""(?:开启|打开|关闭|关掉|切换)(?:蓝牙|wifi|飞行模式|定位|定位服务|手电筒|夜览|勿扰|旋转锁定|热点)") to { text ->
            VoiceIntent.ToggleSetting(text.trim(), text)
        },
        Regex("""(?:截图|截屏|截取屏幕)") to { text ->
            VoiceIntent.TakeScreenshot(text)
        },
        Regex("""(?:打电话|拨打电话|致电|打给)(.+?)") to { text, match ->
            val contact = match?.groups?.get(1)?.value?.trim() ?: ""
            VoiceIntent.MakeCall(contact, text)
        },
        Regex("""(?:发消息|发送消息|发短信|微信|发信息)(?:给)?(.+?)(?:说|道|：)?(.+)") to { text, match ->
            val contact = match?.groups?.get(1)?.value?.trim() ?: ""
            val msg = match?.groups?.get(2)?.value?.trim() ?: ""
            VoiceIntent.SendMsg(contact, msg, text)
        }
    )
    ...
    fun recognize(text: String): VoiceIntent = runCatching {
        withContext(Dispatchers.Default) {
            for ((regex, factory) in patterns) {
                val match = regex.matchEntire(text) ?: regex.find(text)
                if (match != null) {
                    return@runCatching factory(text, match)
                }
            }
...
internal fun Regex.matchEntire(text: String): MatchResult? = find(text)
