    ...
    Column(modifier = Modifier.fillMaxSize().background(MaterialTheme.colorScheme.background)) {
        ...
        Row(
            ...
            FilterChip(
                ...
                label = { Text("文件", fontSize = 12.sp) }
            )
            FilterChip(
                selected = currentTab == 1,
                onClick = { currentTab = 1 },
                label = { Text("构建", fontSize = 12.sp) }
            )
            FilterChip(
                selected = currentTab == 2,
                onClick = { currentTab = 2 },
                label = { Text("输出", fontSize = 12.sp) }
            )
        }
        ...
        when (currentTab) {
            ...
            2 -> OutputTab(outputLog = outputLog)
        }
    }
...
private fun BuildTab(
    ...
    Column(
        ...
        Text("构建 Android APK", fontWeight = FontWeight.Bold, fontSize = 16.sp)
        Text(
            "确保项目根目录包含 gradlew 和 build.gradle 文件。",
...
@Composable
private fun OutputTab(outputLog: String) {
    Column(modifier = Modifier.fillMaxSize().padding(12.dp)) {
...
@Composable
private fun DependencyDialog(engine: TerminalEngine, onDismiss: () -> Unit) {
    val scope = rememberCoroutineScope()
                            ...
                            Button(
                                onClick = {
                                    ...
                                    scope.launch {
                                        ...
                                        for (tool in DevTools.androidTools) {
                                            cmds.addAll(DevTools.getInstallCommand(tool, backend))
                                        }
                                        for (cmd in cmds) {
                            ...
                            Surface(
                                ...
                                Row(
                                    ...
                                    Column(modifier = Modifier.weight(1f)) {
                                        Text(tool.name, fontWeight = FontWeight.Medium, fontSize = 12.sp)
