package com.lingxi.ai.ui.account
...
import com.lingxi.ai.account.LingXiApi
import com.lingxi.ai.account.LingXiClient
import com.lingxi.ai.account.LingXiHosts
import com.lingxi.ai.account.OfficialLoginRequest
import com.lingxi.ai.account.LingXiTokenResponse
import kotlinx.coroutines.launch

/**
 * 灵犀「账户」Tab(对齐策划书二/三/13.7):
 * - 普通用户:QQ/微信 登录(此处仅展示,真实 OAuth 由 SDK 完成)
 * - 官方账号:「官方登录」页,输入 ID + 账号名称 + 密码三字段
 *   成功即 role=ADMIN,加载管理员后台代码
 */
@Composable
fun AccountScreen(app: LingXiApp) {
    var showOfficial by remember { mutableStateOf(false) }
    var loginResult by remember { mutableStateOf<LingXiTokenResponse?>(null) }
...
@Composable
private fun OfficialLoginForm(app: LingXiApp, onSuccess: (LingXiTokenResponse) -> Unit) {
    ...
    Column(
        ...
        Button(
            ...
            onClick = {
                ...
                scope.launch {
                    try {
                        val api: LingXiApi = LingXiClient.create(LingXiHosts.DEFAULT_BASE)
