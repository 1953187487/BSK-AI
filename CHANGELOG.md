...
## v2.0.0（Android Dev Toolkit 正式版）
- 彻底重写，转型为 Android 软件开发工具
- C++ 编辑器为主页，支持 Android 全栈项目开发
- 首次启动协议 + 一键 ATt 依赖安装
- 抽屉导航：项目主页 / 设置
- 项目创建三圆球向导：选文件夹 / 配置 AI 服务商 / 检查依赖
- 支持创建 Android 项目，自定义 API 版本与语言
- 编辑器四按钮：文件 / 搜索项目代码 / AI 辅助 / 同步 GitHub
- GitHub 登录与远程项目下载、源码同步
- 构建与发布：测试安装 / 版本发布，自动全链汉化
- 去除旧版 AI 聊天浮窗等遗留模块
